      ...
      <div className="container px-5 py-24 mx-auto">
        <div className="flex flex-wrap -m-4 justify-center">
          {Object.keys(products).map((item) => {
            // console.log(item._id, item.price) passhref={true}
            return (
              <div key={products[item]._id} className="lg:w-1/4 md:w-1/2 p-4 w-full cursor-pointer shadow-lg m-5">
...
export const getServerSideProps = async () => {
  ...
  for (let item of products) {
    ...
    if (item.title in tshirts) {
      ...
      if (tshirts[item.title].size.includes(item.size) && tshirts[item.title].color.includes(item.color)) {
